...
classes_to_filter = [
    'None']  # You can give list of classes to filter by name, Be happy you don't have to put class number. ['train','person' ]
opt = {
...
source_image_path = "../Example_Images/test02.jpg"
save_path = '../Example_Images_Det'
with torch.no_grad():
    ...
    for i, det in enumerate(pred):
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                plot_one_box(xyxy, img0, label=label, color=1, line_thickness=1)  # color=colors[int(cls)]

    cv2.imwrite(os.path.join(save_path, os.path.split(source_image_path)[-1]), img0)
    cv2.imshow("This is the test", img0)
    cv2.waitKey(0)
